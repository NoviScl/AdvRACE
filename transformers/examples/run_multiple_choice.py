...
class AlbertForMultipleChoice(AlbertPreTrainedModel):
    ...
    def forward(
        ...
        num_choices = input_ids.shape[1] #need some trick for batch-running C3 since number of options is different
...
def evaluate(args, model, tokenizer, prefix="", test=False):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        # eg_idx = -1
        # for pred, correct in zip(preds, out_label_ids):
        #     eg_idx += 1
        #     eg = examples[eg_idx]
        #     if pred != correct:
        #         logger.info("Example ID: {}".format(eg.example_id))
        #         logger.info("Question: {}".format(eg.question))
        #         logger.info("Correct option: {}".format(eg.endings[correct]))
        #         logger.info("Predicted option: {}".format(eg.endings[pred]))
        #         logger.info("Context: {}".format(eg.contexts[0]))
        #         logger.info("\n\n")
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        features = convert_examples_to_features(
            ...
            #distractor_file='/ps2/intern/clsi/output_albert_spanRACE_nbest/nbest_predictions_.json',
            #record_file='/ps2/intern/clsi/output_albert_spanRACE_nbest/PQA_file.json'
        )
        if args.local_rank in [-1, 0]:
...
def main():
    ...
    # Other parameters
    parser.add_argument("--distractor_mode", default=None, type=str, help="Support: random, extract, random_sent")
    parser.add_argument("--attack_mode", default=None, type=str, help="Support: addsent2pas-shuffle, addsent2opt-shuffle, addans2opt-shuffle")
    parser.add_argument(
