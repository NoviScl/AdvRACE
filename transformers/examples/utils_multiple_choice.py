...
def convert_examples_to_features(
    ...
    Note that the last four arguments are added for analysis purposes.
    You can ignore them.
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        # select distractors
        if distractor_mode and distractor_mode.lower() == 'extractive':
            with open(distractor_file, "r", encoding="utf-8") as fin:
                dis_data = json.load(fin)
                if example.example_id not in dis_data:
                    logger.info(example.example_id + ' not found in nbest file. Skipped.')
                    break 
        ...
        dis_seq = question 
        ## should exclude the answer from the list
        for dis in distractors:
            dis_seq = dis_seq + ' ' + dis 
        dis_seq = dis_seq.split()
        random.shuffle(dis_seq)
        dis_seq = ' '.join(dis_seq)

        # use the same distracting sentence
        if attack_mode == 'addsent':
            dis_rand = random.choice(distractors)

        for ending_idx, (context, ending) in enumerate(zip(example.contexts, example.endings)):
            text_a = context
            if attack_mode:
                if attack_mode.lower() == 'addsent2pas-shuffle':
                    text_a = text_a + ' ' + dis_seq

                ## AddSent: Append Q+A at the end
                if attack_mode.lower() == 'addsent':
                    #text_a = text_a + ' ' + question + ' ' + dis_rand
                    text_a = text_a + ' ' + question

                if attack_mode.lower() == 'addsent2opt-shuffle' and ending_idx!=label:
                    sents = find_sentences(context)
                    sent = random.choice(sents)
                    ending = sent.split() + ending.split()
                    random.shuffle(ending)
                    ending = ' '.join(ending)

                if attack_mode.lower() == 'addans2opt-shuffle' and ending_idx!=label:
                    ending = answer.split() + ending.split()
                    random.shuffle(ending)
                    ending = ' '.join(ending)

                if attack_mode.lower() == 'addsent2opt' and ending_idx!=label:
                    sents = find_sentences(context)
                    sent = random.choice(sents)
                    ending = sent.split() + ending.split()
                    #random.shuffle(ending)
                ...
                if attack_mode.lower() == 'addans2opt' and ending_idx!=label:
                    ending = answer.split() + ending.split()
                    #random.shuffle(ending)
                    ending = ' '.join(ending)

                if attack_mode.lower() == 'addsent2opt-end' and ending_idx!=label:
                    sents = find_sentences(context)
                    sent = random.choice(sents)
                    ending = ending.split() + sent.split()
                    #random.shuffle(ending)
                    ending = ' '.join(ending)

                if attack_mode.lower() == 'addans2opt-end' and ending_idx!=label:
                    ending = ending.split() + answer.split()
                    #random.shuffle(ending)
                    ending = ' '.join(ending)

                if attack_mode.lower() == 'sent2opt-shuffle' and ending_idx!=label:
                    sents = find_sentences(context)
                    sent = random.choice(sents)
                    ending = sent.split()
                    random.shuffle(ending)
                ...
                if attack_mode.lower() == 'ans2opt-shuffle' and ending_idx!=label:
                    ending = answer.split() 
                    random.shuffle(ending)
                    ending = ' '.join(ending)

            if distractor_mode:
                if distractor_mode.lower() == 'random' and ending_idx != int(example.label):
                    ans_len = len(ending.split())
                    context_tok = context.replace('\n', ' ').split()
                    dis_pos = random.randrange(0, len(context_tok)-ans_len)
                    # while dis_pos in rand_spans:
                    #     dis_pos = random.randrange(0, len(context_tok)-ans_len)
                    # rand_spans.append(dis_pos)
                    dis = ' '.join(context_tok[dis_pos : dis_pos+ans_len])
                    #print (dis)
                    ending = dis
                elif distractor_mode.lower() == 'extractive' and ending_idx != int(example.label):
                    if len(ext_spans) > 0:
                        ending = ext_spans[0]
                        # print (ending)
                        ext_spans.pop(0)
                        eg_dict['distractors'].append(ending)
                elif distractor_mode.lower() == 'random_sent' and ending_idx != int(example.label):
                    context_sent = find_sentences(context.replace('\n', ' '))
                    dis = random.choice(context_sent)
                    ending = dis
                    #print (ending)
