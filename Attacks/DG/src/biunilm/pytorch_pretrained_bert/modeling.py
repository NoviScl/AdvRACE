...
class BertForSeq2SeqDecoder(PreTrainedBertModel):
    ...
    def __init__(self, config, mask_word_id=0, num_labels=2, num_rel=0,
                 ...
                 forbid_duplicate_ngrams=False, forbid_ignore_set=None, not_predict_set=None, ngram_size=3, min_len=0, mode="s2s", pos_shift=False, topk=10):
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None):
        ...
        while next_pos < output_length:
            ...
            prediction_scores = torch.squeeze(prediction_scores, dim=1) #(bsz, vocab)
            #print (prediction_scores.size())
            # topk_values, topk_indices = torch.topk(prediction_scores, k=self.topk, dim=-1) #(bsz, k)
            #print (topk_values.size())
            #print (topk_indices.size())
            #samples = torch.multinomial(topk_values, num_samples=1) #(bsz, 1)
            #max_ids = torch.gather(topk_indices, -1, samples) #(bsz, 1)
    ...
    def beam_search(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None):
        ...
        # [(batch, beam)]
        total_scores = [x.tolist() for x in total_scores] #shape: (len, batch_size, beam_size)
        # print ('total_scores shape: ', np.array(total_scores).shape)
        ...
        for b in range(batch_size):
            # [(beam,)]
            scores = [x[b] for x in total_scores] #(len, beam)
            ...
            pos_in_frame = -1
            """
            Modified here, we keep the top K(=beam size) most probable sequences,
            and store them in trace['pred_seq'].
            """
            max_dict = []

            # find sequences with max probability
            # each time step
            for fid in range(last_frame_id + 1):
                for i, wid in enumerate(wids_list[fid]):
                    if wid == self.eos_id or fid == last_frame_id:
                        ...
                        #     frame_id = fid
                        #     pos_in_frame = i
                        max_dict.append({'score': s, 'frame_id': fid, 'pos_in_frame': i})
            max_dict = sorted(max_dict, key=lambda x: x['score'], reverse=True)
            max_dict = max_dict[ : K] #(only keep top K seq)
            # if frame_id == -1:
            #     traces['pred_seq'].append([0])
            #else:
            for seq_dict in max_dict:
                frame_id = seq_dict['frame_id']
                pos_in_frame = seq_dict['pos_in_frame']
