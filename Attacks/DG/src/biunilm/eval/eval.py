...
class Eval:
    ...
    def evaluate(self):
        logger.info("="*5 +"MSCOCO Evaluation Script: "+"="*5)
        output = []
        scorers = [
...
def eval(hyp, ref):
    ...
    logger.info("*=" * 10+"First Distractor"+"*=" * 10)
    DGEval1 = Eval(gts, res1)
    eval1 = list(map(lambda x: x * 100, DGEval1.evaluate()))
    logger.info("B1: {:.2f}, B2: {:.2f}, B3: {:.2f}, B4: {:.2f}".format(
        eval1[0], eval1[1], eval1[2], eval1[3]
    ))


    logger.info("*=" * 10+"Second Distractor"+"*=" * 10)
    ...
    logger.info("*=" * 10+"Third Distractor"+"*=" * 10)
