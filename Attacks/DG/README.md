...
Note: We have also experimented with directly using all the distractors in RACE for training.  But it turns out that using the cleaned version provided by Yifan Gao gives better generation quality. So please go download the processed dataset from his repo first, then convert the data into UniLM src-tgt format. Then use run_unilm.sh and run_decode.sh for training and decoding.

The original UniLM instructions are below.
